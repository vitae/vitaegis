...
function ScopeMessage({ children }: { children: React.ReactNode }) {
  return (
    <div className={`flex h-full w-full items-center justify-center text-vitae-green/70 ${hud}`}>
      {children}
    </div>
  );
}
      ...
      >
        {[0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330].map((deg) => {
          ...
          return (
            <text
              key={deg}
              x={(100 + Math.cos(a) * 91.5).toFixed(2)}
              y={(100 + Math.sin(a) * 91.5 + 1.3).toFixed(2)}
            >
...
export default function RadarConsole({
  ...
  const active = useMemo(
    () => (region ? legs.map((l) => l.region === region) : undefined),
    [legs, region],
  );
  const shown = useMemo(
    () => legs.map((l, i) => i).filter((i) => !active || active[i]),
    [legs, active],
  );
            ...
            {regions.map((r) => {
              ...
              const body = (
                <>
                  ...
                  >
                    <span>
                      {r.rank ? `#${r.rank}` : `${count} ${count === 1 ? 'track' : 'tracks'}`}
                    </span>
                    {r.miles !== undefined && <span>{r.miles.toLocaleString('en-US')} mi</span>}
                  </span>
                  <span className="mt-1 block text-base font-semibold tracking-[0.08em] text-white">
                    {r.label}
                  </span>
                  {r.rank && (
                  ...
                  {r.blurb && (
                    <span className="mt-1.5 hidden text-xs font-light leading-relaxed text-white/55 lg:block">
                      {r.blurb}
                    </span>
                  )}
                  {r.href && (
                    <span
                      className={`mt-2 block ${on ? 'text-white' : 'text-white/40 group-hover:text-white'} ${hud}`}
                    >
                      Open →
                    </span>
                  )}
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)] lg:items-start lg:gap-8">
        ...
        <div className="relative mx-auto w-full max-w-[640px]">
          <div className={`mb-3 flex items-start justify-between gap-4 text-vitae-green ${hud}`}>
            <div>
              ...
              <p className="mt-1 text-white/45">
                {scope} · {shown.length} tracks{picked ? ' lit' : ''}
                <span className="hidden lg:inline"> · drag to turn · click a track to lock</span>
              </p>
            </div>
            <div className="text-right tabular-nums">
              <p>
                ...
                <span className="text-white">
                  {now ? clock(airports[home].tz, now) : '--:--:--'}
                </span>
              </p>
              <p className="mt-1 text-white/45">
                {leg && destZone ? (
                  <>
                    ...
                    <span className="text-[#ff00ff]">
                      {now ? clock(destZone, now) : '--:--:--'}
                    </span>
                  </>
                ) : (
                  <>
                    UTC{' '}
                    <span className="text-white/70">{now ? clock('UTC', now) : '--:--:--'}</span>
                  </>
                )}
              </p>
            </div>
          ...
          >
            ...
            {!still && (
              <div
                className="travel-sweep pointer-events-none absolute inset-0 rounded-full"
                aria-hidden
              />
            )}
            <div
              className="travel-scanlines pointer-events-none absolute inset-0 rounded-full"
              aria-hidden
            />
            <ScopeChrome />
          </div>
          ...
          <div
            className={`mt-3 flex flex-wrap items-center justify-between gap-x-6 gap-y-2 text-white/55 ${hud}`}
          >
            ...
            {rejected && (
              <span className="flex items-center gap-2">
                <i className="h-px w-6 border-t border-dashed border-[#ff0000]" /> Rejected ·{' '}
                {rejected.from}→{rejected.to} direct
              </span>
            )}
            <span className="flex items-center gap-2">
              <i className="h-2 w-2 border border-vitae-green/80 shadow-[0_0_6px_#00ff00]" /> Track
              · FL · GS · HDG · ETA
            </span>
          </div>
        </div>
                ...
                <li key={l.n}>
                  ...
                  >
                    <span className="flex items-baseline gap-3">
                      <span
                        style={tint}
                        className={`w-5 text-xs tabular-nums ${on ? 'text-[#ff00ff]' : hv ? 'text-white' : 'text-vitae-green/70'}`}
                      >
                        {l.n}
                      </span>
                      <span className="text-base font-semibold tracking-[0.12em] text-white sm:text-lg">
                        ...
                        <span
                          style={tint}
                          className={`mx-2 ${on ? 'text-[#ff00ff]' : hv ? 'text-white' : 'text-vitae-green'}`}
                        >
                          →
                        </span>
                        {l.toLabel ?? l.to}
                      ...
                      <span
                        style={tint}
                        className="ml-auto w-14 text-right text-base font-semibold tabular-nums text-vitae-green sm:ml-0"
                      >
                        ${l.farePP}
                      </span>
                    </span>
                    ...
                    {on && (
                      <span className="mt-2 block border-t border-[#ff00ff]/25 pl-8 pt-2 text-sm font-light leading-relaxed text-white/80">
                        ...
                        <span
                          className={`mt-1.5 block ${l.flag === 'visa' ? 'text-[#ffff00]' : ''}`}
                        >
                          {l.note}
                        </span>
                        {l.via && (
                          <span className={`mt-1.5 block text-white/40 ${hud}`}>
                            Routing {waypoints(l).join(' → ')}
                          </span>
                        )}
                      </span>
                    )}
