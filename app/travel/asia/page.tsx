...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: 'The Pacific Circuit | VITAEGIS Travel',
    description:
      'Six countries, eight flights, one loop out of Honolulu — routed for the lowest fare.',
    type: 'article',
  },
};
...
function SectionHead({ kicker, title, sub }: { kicker: string; title: string; sub?: string }) {
  return (
    <header className="text-center">
      ...
      {sub && (
        <p className="mx-auto mt-2 max-w-2xl text-lg font-light italic text-white/60">{sub}</p>
      )}
    </header>
  );
}
...
const seasonCell = [
  'bg-white/[0.04]',
  'bg-vitae-green/25',
  'bg-vitae-green shadow-[0_0_10px_rgba(0,255,0,0.5)]',
];
      ...
      <div className="relative mx-auto max-w-6xl px-4 pb-32 pt-10 sm:px-6">
        ...
        <div className="mt-12 space-y-12">
          ...
          <section id="insight" className={`${glass} scroll-mt-10 p-6 sm:p-10`}>
            ...
            <div className="mt-8 grid items-center gap-8 md:grid-cols-[1.4fr_1fr]">
              <p className="font-light leading-relaxed text-white/75">
                ...
                competition, and the fare shows it. Break the same crossing at Seoul and it lands on
                two fought-over routes instead: several Asian carriers compete for Bali–Seoul, and
                Seoul–Honolulu is an established daily nonstop. The detour is the cheaper way to
                make the same crossing, and it adds a sixth country.
              </p>
              <div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-white/10 bg-white/10 text-center">
                <div className="bg-black p-5">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[#ff0000]">
                    Direct-ish
                  </p>
                ...
                <div className="bg-black p-5">
                  ...
                  <p className="mt-2 text-3xl font-semibold tabular-nums text-vitae-green">
                    {usd(rejected.viaSeoulPP)}
                  </p>
                  <p className="mt-1 text-xs font-light text-white/50">
                    DPS → ICN → HNL, per person
                  </p>
                </div>
              </div>
            </div>
          ...
          <section id="stops" className="scroll-mt-10">
            ...
            <div className="mt-8 space-y-6">
              {stops.map((stop, i) => (
                <article
                  key={stop.slug}
                  id={stop.slug}
                  className={`${glass} scroll-mt-10 p-6 sm:p-10`}
                >
                  <header className="flex flex-col gap-2 border-b border-vitae-green/20 pb-5 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                      ...
                      <h3 className="mt-1 text-3xl font-semibold tracking-wide text-white">
                        {stop.city}
                      </h3>
                    ...
                    <div className="flex gap-6 sm:text-right">
                      ...
                      <div>
                        ...
                        <p className="mt-1 whitespace-nowrap tabular-nums text-white/85">
                          {stop.dailyForTwo}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="mt-6 grid gap-8 md:grid-cols-2">
                    ...
                    <div className="space-y-5">
                      <div>
                        ...
                        <ul className="mt-2 space-y-2">
                          {stop.doThis.map((item) => (
                            <li
                              key={item}
                              className="flex gap-3 font-light leading-relaxed text-white/80"
                            >
                              <span
                                aria-hidden
                                className="mt-[0.6em] h-1.5 w-1.5 flex-none rounded-full bg-vitae-green shadow-[0_0_6px_#00ff00]"
                              />
                              {item}
                            </li>
                          ))}
                      ...
                      <div>
                        ...
                        <p className="mt-1 font-light leading-relaxed text-white/80">
                          {stop.eatThis}
                        </p>
                      </div>
                    </div>
                  ...
                  <p className="mt-6 border-t border-vitae-green/20 pt-5 text-sm font-light leading-relaxed text-white/65">
                    <span className="mr-2 text-[11px] font-semibold uppercase tracking-[0.25em] text-[#ffff00]">
                      Watch out
                    </span>
                    {stop.watchOut}
                  </p>
            ...
            <p className="mt-5 text-center text-sm font-light italic text-white/50">
              Daily budgets are mid-range estimates for two people sharing a room — lodging, food,
              local transport and entry fees, flights excluded.
            </p>
          </section>
            ...
            <div className="mt-8 overflow-x-auto">
              <table className="w-full min-w-[560px] border-separate border-spacing-y-2 text-left">
                <thead>
                  <tr>
                    ...
                    {monthLetters.map((m, i) => (
                      <th
                        key={i}
                        className={`text-center text-xs font-medium ${i === 9 || i === 10 ? 'text-vitae-green' : 'text-white/45'}`}
                      >
                        {m}
                      </th>
                    ))}
                ...
                <tbody>
                  {seasons.map((row) => (
                    <tr key={row.label}>
                      <th scope="row" className="pr-3 text-sm font-medium text-white/85">
                        {row.label}
                      </th>
                      ...
                      <td className="hidden pl-4 text-sm font-light text-white/55 lg:table-cell">
                        {row.why}
                      </td>
                    </tr>
                  ))}
          ...
          <section id="paperwork" className={`${glass} scroll-mt-10 p-6 sm:p-10`}>
            ...
            <ul className="mt-8 divide-y divide-white/10 border-y border-vitae-green/20">
              {paperwork.map((row) => (
                <li
                  key={row.country}
                  className="grid gap-x-6 gap-y-1 py-4 sm:grid-cols-[1fr_1.4fr_0.9fr_2fr] sm:items-baseline"
                >
                  ...
                  <p className="text-sm font-light text-white/65">
                    {row.ahead && (
                      <span className="mr-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#ffff00]">
                        Before you fly
                      </span>
                    )}
                    {row.file}
            ...
            <p className="mt-5 text-center text-sm font-light italic text-white/55">
              About $135 in fees for two. Passports need six months of validity past each entry date
              and a couple of blank pages. Rules change — Thailand’s did this month — so reconfirm
              each on the country’s official immigration site before booking.
            </p>
          </section>
          ...
          <section id="tigers" className={`${glass} scroll-mt-10 p-6 sm:p-10`}>
            ...
            <p className="mt-5 text-center text-sm font-light italic text-white/55">
              ...
              breeding, training and housing. Worth weighing next to the price and the reviews.
              Prices and ratings as listed by each venue’s booking pages — reconfirm before you go.
            </p>
          </section>
            ...
            <div className="mt-8 grid gap-x-10 gap-y-8 md:grid-cols-2">
              {tactics.map((t, i) => (
                <div key={t.head} className="grid grid-cols-[2rem_1fr] gap-x-2">
                  <span className="pt-0.5 text-sm font-semibold tabular-nums text-vitae-green">
        ...
        <footer className="mt-16 text-center text-sm font-light text-white/40">
          <p className="mx-auto max-w-3xl">
            Built for two travelers leaving from and returning to Honolulu. Fares are indicative
