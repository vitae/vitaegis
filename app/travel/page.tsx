      ...
      <div className="relative mx-auto max-w-6xl px-4 pb-32 pt-10 sm:px-6">
        ...
        <header className="pb-10 pt-12 text-center sm:pt-16">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-lg font-light text-white/70">
            Every track starts in Honolulu, on Alaska Airlines, Hawaiian, American and their
            oneworld partners. Common flight paths through Asia, South America, Alaska, Dallas,
            Switzerland and Reykjavik — nonstops first, then ranked nearest to farthest. Pick a
            sector to light its tracks; click it to open that sector’s own page. Pick a strip to
            lock onto a flight.{' '}
        ...
        <section className="mt-12">
          <header className="text-center">
            ...
            <p className="mx-auto mt-2 max-w-2xl text-lg font-light italic text-white/60">
              Sectors with a nonstop from Honolulu rank first. Miles are direct great-circle
              distance to each sector’s closest destination; strips run nonstops first, then
              shortest-first by miles actually flown.
            </p>
          </header>
            ...
            {sectors.map((s) => {
              ...
              return (
                <li key={s.key} className={`${glass} p-6`}>
                  ...
                  >
                    <p className={label} style={s.color ? { color: s.color } : undefined}>
                      #{s.rank} · {s.sector}
                    </p>
                    <p
                      className="text-sm tabular-nums text-vitae-green"
                      style={s.color ? { color: s.color } : undefined}
                    >
                      {s.miles.toLocaleString('en-US')} mi
                    </p>
                  </div>
                  ...
                  <p className="mt-1 text-sm font-light text-white/55">
                    Shortest track: HNL → {nearest.to}
                    {nearest.via ? ` via ${nearest.via.join(', ')}` : ''} ·{' '}
                    {legMiles(airports, nearest).toLocaleString('en-US')} mi flown
                  </p>
