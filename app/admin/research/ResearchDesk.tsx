...
export default function ResearchDesk() {
  return (
    <AdminShell
      ...
      blurb={
        <>
          Feed in books, papers, YouTube videos and articles. Gemini reads each one end to end and
          keeps the pertinent quotes and the scientific findings. Build a brief per topic to see
          what recurs across sources, then send it to the post queue: it lands in{' '}
...
function Desk({ adminKey: key }: { adminKey: string }) {
  ...
  const headers = useCallback(
    (k: string) => ({ 'x-admin-key': k, 'Content-Type': 'application/json' }),
    [],
  );
    ...
    async (k: string) => {
      ...
      try {
        const [s, f, b] = await Promise.all([
          ...
          fetch(`/api/research/findings${filterTopic ? `?topic=${filterTopic}` : ''}`, {
            headers: headers(k),
            cache: 'no-store',
          }),
  ...
  useEffect(() => {
    const inFlight =
      sources.some((s) => s.status === 'queued' || s.status === 'extracting') ||
      briefs.some((b) => b.status === 'queued');
    if (!inFlight) return;
  ...
  const addSource = async () => {
    ...
    try {
      ...
      if (file) {
        const sign = (await post('/api/research/upload', {
          name: file.name,
          size: file.size,
          type: file.type,
        })) as {
  ...
  const when = (iso: string) =>
    new Date(iso).toLocaleString('en-US', { timeZone: 'Pacific/Honolulu' });
      ...
      <>
        ...
        <section className={`${glass} mt-8 p-5 sm:p-7`}>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <label className="block sm:col-span-2">
              <span className="text-[10px] uppercase tracking-[0.2em] text-white/40">
                or a PDF (book, paper — up to 50 MB)
              </span>
              <input
                ref={fileRef}
                type="file"
          ...
          <div className="mt-5 flex items-center gap-3">
            ...
            <span className="text-xs text-white/40">
              Extraction runs in the background; this list refreshes itself.
            </span>
          </div>
        </section>
        ...
        {tab === 'sources' && (
          <div className="mt-6 space-y-4">
            {sources.length === 0 && (
              <p className="text-sm font-light text-white/50">Nothing read yet.</p>
            )}
            {sources.map((s) => (
              <article key={s.id} className={`${glass} p-5`}>
                <header className="flex flex-wrap items-baseline justify-between gap-2">
                  <div>
                    <span className={`${label} ${TONE[s.status] ?? 'text-white/50'}`}>
                      {s.status}
                    </span>
                ...
                {s.url && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-1 block truncate text-xs text-vitae-green/70 hover:text-vitae-green"
                  >
                    {s.url}
                  </a>
                )}
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {s.status === 'done' && (
                    <button
                      onClick={() => {
                        ...
                        setFilterTopic('');
                        setFindings([]);
                        void fetch(`/api/research/findings?source=${s.id}`, {
                          headers: headers(key),
                          cache: 'no-store',
                        })
                          .then((r) => r.json())
                  ...
                  {(s.status === 'failed' || s.status === 'done') && (
                    <button
                      disabled={busy === s.id}
                      onClick={() =>
                        act('/api/research/sources', { action: 'retry', id: s.id }, s.id)
                      }
                      className={btnQuiet}
                    >
                  ...
                  <button
                    disabled={busy === s.id}
                    onClick={() =>
                      confirm('Delete this source and everything pulled from it?') &&
                      act('/api/research/sources', { action: 'delete', id: s.id }, s.id)
                    }
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 space-y-3">
              {shownFindings.length === 0 && (
                <p className="text-sm font-light text-white/50">Nothing here yet.</p>
              )}
              {shownFindings.map((f) => (
                <article key={f.id} className={`${glass} p-4`}>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={`${label} ${f.kind === 'quote' ? 'text-white/70' : ''}`}>
                      {f.kind}
                    </span>
                  ...
                  <p
                    className={`mt-2 text-sm ${f.kind === 'quote' ? 'italic text-white' : 'text-white/90'}`}
                  >
          ...
          <div className="mt-6">
            <div className={`${glass} p-5`}>
              ...
              <p className="mt-2 text-xs text-white/50">
                Collates every quote and finding tagged with the topic, ranks what recurs across
                sources, keeps the best quotes.
              </p>
              <div className="mt-3 flex flex-wrap items-center gap-2">
                {TOPICS.map((t) => (
                  <button
                    key={t}
                    onClick={() => setBriefTopic(t)}
                    className={pill(briefTopic === t)}
                  >
                    {t}
                  </button>
                ))}
                <button
                  disabled={busy === 'build'}
                  onClick={() =>
                    act('/api/research/briefs', { action: 'build', topic: briefTopic }, 'build')
                  }
                  className={`${btn} ml-2`}
                >
            ...
            <div className="mt-6 space-y-5">
              {briefs.length === 0 && (
                <p className="text-sm font-light text-white/50">No briefs yet.</p>
              )}
              {briefs.map((b) => (
                <article key={b.id} className={`${glass} p-5 sm:p-7`}>
                  <header className="flex flex-wrap items-baseline justify-between gap-2">
                    <div>
                      <span className={`${label} ${TONE[b.status] ?? 'text-white/50'}`}>
                        {b.status}
                      </span>
                      <span className="ml-3 text-[10px] uppercase tracking-[0.2em] text-white/40">
                        {b.topic}
                  ...
                  {b.takeaway && (
                    <p className="mt-4 text-sm text-white/70">Do this week: {b.takeaway}</p>
                  )}
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {b.status === 'ready' && (
                      <button
                        disabled={busy === b.id}
                        onClick={() =>
                          act('/api/research/briefs', { action: 'post', id: b.id }, b.id)
                        }
                        className={btn}
                      >
                        Send to post queue
                    ...
                    {b.status !== 'queued' && (
                      <button
                        disabled={busy === b.id}
                        onClick={() =>
                          act('/api/research/briefs', { action: 'rebuild', id: b.id }, b.id)
                        }
                        className={btnQuiet}
                      >
                        Rebuild
                    ...
                    <button
                      disabled={busy === b.id}
                      onClick={() =>
                        confirm('Delete this brief?') &&
                        act('/api/research/briefs', { action: 'delete', id: b.id }, b.id)
                      }
                      className="rounded-lg border border-red-500/40 px-3 py-1.5 text-xs text-red-400 hover:bg-red-500/10 disabled:opacity-40"
                    >
                      Delete
