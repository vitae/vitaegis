...
function Stat({
  ...
  const body = (
    <div
      className={`${ui.glass} p-4 transition-colors ${href ? 'hover:border-vitae-green/50' : ''}`}
    >
...
export default function AdminDashboard() {
  return (
    <AdminShell
      title="Dashboard"
      blurb="What is waiting on you, what went out, and whether the pipeline is running."
    >
...
function Body({ adminKey }: { adminKey: string }) {
  ...
  const workerStale = data.jobs.last
    ? Date.now() - new Date(data.jobs.last.updated_at).getTime() > 15 * 60_000
    : false;
    ...
    <div className="mt-8 space-y-8">
      ...
      <section>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Stat
            ...
            value={data.posts.needsReview}
            href="/admin/content?status=needs"
            tone="text-vitae-green"
          />
          <Stat
            label="Failed"
            value={data.posts.failed}
            href="/admin/content?status=failed"
            tone={data.posts.failed ? 'text-red-400' : 'text-white'}
          />
          <Stat
            label="Briefs ready to post"
            value={data.research.briefsReady}
            href="/admin/research"
            tone="text-vitae-green"
          />
      ...
      <section>
        ...
        <div className="mt-3 grid grid-cols-3 gap-3">
          <Stat
            label="Last 7 days"
            value={data.posts.published7}
            href="/admin/content?status=published"
          />
          <Stat
            label="Last 30 days"
            value={data.posts.published30}
            href="/admin/content?status=published"
          />
          <Stat
            label="All time"
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.accounts.map((a) => {
                ...
                const expiring =
                  a.expires_at && a.expires_at * 1000 - Date.now() < 3 * 24 * 60 * 60 * 1000;
                return (
                  <tr key={a.platform} className="border-t border-white/5">
                    ...
                    <td className="py-2">
                      {a.connected ? (
                        ...
                        <a
                          href={`/api/social/connect/${a.platform}?key=${encodeURIComponent(adminKey)}`}
      ...
      <section>
        <div className="flex items-baseline justify-between">
          ...
          <Link
            href="/admin/content?status=published"
            className="text-xs text-white/50 hover:text-vitae-green"
          >
            Full history →
          </Link>
        </div>
      ...
      <section>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Stat
            ...
            tone={data.jobs.failed ? 'text-red-400' : 'text-white'}
          />
        ...
        <div className={`${ui.glass} mt-3 p-4 text-sm`}>
          <p className={workerStale && pending ? 'text-yellow-300' : 'text-white/70'}>
            ...
            {workerStale && pending
              ? ' — jobs are waiting but the cron has been quiet for over 15 minutes.'
              : ''}
          </p>
                ...
                <li key={j.id} className="text-xs text-red-300">
                  ...
                  {j.post_id && (
                    <>
                      {' · '}
                      <Link
                        href={`/admin/content?q=&id=${j.post_id}`}
      ...
      <p className="text-xs text-white/30">
        Updated {hst(data.generatedAt)} HST · refreshes every 30 seconds
      </p>
    </div>
  );
}
