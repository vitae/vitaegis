...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await db
    ...
    .select(
      'id, kind, title, author, url, storage_path, note, topics, status, summary, error, findings_count, created_at',
    )
    .order('created_at', { ascending: false })
    .limit(200);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(
    { sources: data ?? [], topics: TOPICS },
...
export async function POST(req: NextRequest) {
  ...
  if (body.storage_path) kind = 'pdf';
    ...
    return NextResponse.json(
      { error: 'Send a url, an uploaded storage_path, or text' },
      { status: 400 },
    );

  if (url && !/^https?:\/\//i.test(url))
    return NextResponse.json({ error: 'URLs must start with http(s)://' }, { status: 400 });

  const topics = (body.topics ?? []).filter((t): t is (typeof TOPICS)[number] =>
