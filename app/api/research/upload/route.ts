...
export async function POST(req: NextRequest) {
  ...
  const { name, size, type } = (await req.json().catch(() => ({}))) as { name?: string; size?: number; type?: string };
  if (!name || !size) return NextResponse.json({ error: 'Send the file name and size' }, { status: 400 });
  if (size > MAX_BYTES) return NextResponse.json({ error: 'PDFs are capped at 50 MB' }, { status: 413 });
  if (type && type !== 'application/pdf') return NextResponse.json({ error: 'Only PDFs are accepted here' }, { status: 415 });
