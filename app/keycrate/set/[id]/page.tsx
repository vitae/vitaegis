...
import { DEFAULT_SETTINGS, type Camelot, type PlaylistSettings, type Track } from '@/lib/keycrate/types';
...
async function loadSet(id: string) {
  ...
  const { data: playlist } = await sb.from('kc_playlists').select('id, name, mode, target_curve, updated_at').eq('id', id).eq('is_public', true).maybeSingle();
  if (!playlist) return null;
  const { data: items } = await sb
    .from('kc_playlist_items')
    .select('position, note, kc_tracks(id, source_id, artist, title, camelot, bpm, duration_s, genre, label, energy)')
    .eq('playlist_id', id)
    .order('position');
  const tracks: Track[] = (items ?? [])
    .map((it) => it.kc_tracks as unknown as { id: string; source_id: string; artist: string; title: string; camelot: string | null; bpm: number | null; duration_s: number | null; genre: string | null; label: string | null; energy: number | null } | null)
  ...
  return { name: playlist.name as string, updatedAt: playlist.updated_at as string, settings, tracks };
}
...
export default async function SharedSetPage({ params }: Params) {
  ...
  const path = set.tracks.map((t, i) => ({ key: t.camelot, transition: i > 0 ? transitions[i - 1] : null }));
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col px-4 pb-16 pt-4 sm:px-6">
      ...
      <p className="text-xs text-[#808880]">
        {set.tracks.length} tracks · {Math.round(total / 60)} min · {set.settings.mode} mode · shared read-only
      </p>
