...
const { STRAVA_CLIENT_ID: id, STRAVA_CLIENT_SECRET: secret, STRAVA_VERIFY_TOKEN: verify } = process.env;
if (!id || !secret || !verify) throw new Error('Set STRAVA_CLIENT_ID, STRAVA_CLIENT_SECRET and STRAVA_VERIFY_TOKEN in .env.local');
...
if (cmd === 'create') {
  ...
  const body = new URLSearchParams({ client_id: id, client_secret: secret, callback_url: `${arg.replace(/\/$/, '')}/api/strava/webhook`, verify_token: verify });
