...
describe('filterTracks', () => {
  ...
  it('applies every filter', () => {
    ...
    expect(
      r.every(
        (t) =>
          t.camelot === '2A' && t.genre === 'Techno' && (t.rating ?? 0) >= 1 && !used.has(t.id),
      ),
    ).toBe(true);
    expect(r.length).toBeGreaterThan(0);
    const untagged = filterTracks(
      indexTracks([{ ...lib[0], camelot: null }, lib[1]]),
      { ...EMPTY_FILTERS, needsAnalysisOnly: true },
      new Set(),
    );
