...
const EXTRACTION_SCHEMA = {
  ...
  properties: {
    ...
    summary: {
      type: 'string',
      description: 'Three sentences on what this source argues and who it is for.',
    },
    quotes: {
      type: 'array',
      items: {
        ...
        properties: {
          text: {
            type: 'string',
            description: 'Verbatim, exactly as written or spoken. Never paraphrase.',
          },
          location: {
            type: 'string',
            description: 'Page or chapter for documents, MM:SS for video.',
          },
    ...
    findings: {
      ...
      items: {
        ...
        properties: {
          claim: {
            type: 'string',
            description: 'One sentence, plain, specific, no hedging words.',
          },
          evidence: {
            type: 'string',
            description: 'The study, mechanism, sample size, dose or numbers behind it.',
          },
...
) => `You are the research editor for VITAEGIS, a health and wellness channel about maximizing human energy: mitochondria, brainwaves, meditation, fitness, recipes, travel, and camping and hiking gear.
...
async function fetchArticle(url: string): Promise<{ text: string; pdf?: Buffer }> {
  const res = await fetch(url, {
    headers: {
      'User-Agent': 'Mozilla/5.0 (compatible; VitaegisResearch/1.0)',
  ...
  if (type.includes('application/pdf'))
    return { text: '', pdf: Buffer.from(await res.arrayBuffer()) };
  ...
  const text = html
    ...
    .trim();
  if (text.length < 200)
    throw new Error('The page had no readable text (paywall or script-rendered)');
...
async function partsFor(
  source: ResearchSource,
): Promise<{ parts: InteractionPart[]; cleanup: () => Promise<void> }> {
  const noop = async () => {};
  ...
  if (source.kind === 'pdf') {
    ...
    if (!fetched.pdf) {
      return {
        parts: [{ type: 'text', text: `ARTICLE (${source.url}):\n\n${fetched.text}` }],
        cleanup: noop,
      };
    }
    bytes = fetched.pdf;
  ...
  if (bytes.byteLength > 50 * 1024 * 1024)
    throw new Error('PDFs are capped at 50 MB by Gemini; split the book');
  const uploaded = await waitForFile(
    await uploadFile(bytes, mimeType, source.title || source.url || 'source'),
  );
  return {
...
const BRIEF_SCHEMA = {
  ...
  properties: {
    ...
    summary: {
      type: 'string',
      description: 'Under 120 words. The state of the evidence on this topic across the sources.',
    },
    keyFindings: {
      type: 'array',
      items: {
        ...
        properties: {
          ...
          recurrence: {
            type: 'integer',
            description: 'How many distinct sources support this claim.',
          },
          sources: { type: 'array', items: { type: 'string' }, description: 'Source titles.' },
          evidence: {
            type: 'string',
            description: 'The strongest evidence in one line: numbers where they exist.',
          },
        },
...
export async function buildBrief(briefId: string) {
  ...
  let q = db()
    ...
    .select(
      'kind, text, evidence, location, topic, strength, source:research_sources(id, title, author, kind, url)',
    )
  ...
  if (!findings?.length)
    throw new Error(`No findings tagged "${brief.topic}" yet. Extract some sources first.`);
...
export function briefNote(brief: {
  ...
  const findings = (brief.key_findings ?? [])
    .map(
      (f, i) =>
        `${i + 1}. ${f.claim} (${f.recurrence} source${f.recurrence === 1 ? '' : 's'}; ${f.evidence})`,
    )
    .join('\n');
  const quotes = (brief.quotes ?? [])
    .map((q) => `"${q.text}" — ${q.author || q.source}`)
    .join('\n');
